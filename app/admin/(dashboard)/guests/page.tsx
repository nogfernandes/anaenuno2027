...
export default async function Guests(){
  ...
  const guests=(guestData??[]) as unknown as Guest[];const answers=(answerData??[]) as unknown as Answer[];const invitations=(invitationData??[]) as Invitation[];const available=invitations.filter(invitation=>invitation.max_guests<5);
  return <><p className="eyebrow">Everyone</p><h1 className="font-editorial my-6 text-6xl">Guests + answers</h1><p className="mb-8 max-w-xl text-lg leading-6 opacity-70">Rename, add or remove participants, update RSVP details and review their submitted answers.</p>
    <details className="mb-8 bg-[#f8f4ed] p-6"><summary className="cursor-pointer font-editorial text-3xl">Add guest <span className="float-right">+</span></summary>{available.length?<form action={addGuest} className="mt-6 grid gap-5 sm:grid-cols-3"><label className="text-sm uppercase tracking-widest">Invitation<select className="field" name="invitation_id" required>{available.map(invitation=><option value={invitation.id} key={invitation.id}>{invitation.code} · {invitation.max_guests}/5</option>)}</select></label><label className="text-sm uppercase tracking-widest">Guest name<input className="field" name="name" maxLength={150} required/></label><label className="text-sm uppercase tracking-widest">Guest type<select className="field" name="category"><option value="adult">Adult</option><option value="child">Child</option><option value="baby">Baby</option></select></label><button className="button sm:col-span-3 sm:justify-self-start">Add guest</button></form>:<p className="mt-5 text-lg opacity-70">Every active invitation already has five guests.</p>}</details>
    <div className="grid gap-2">{guests.map(g=>{
      ...
      return <details className="group bg-[#f8f4ed]" key={g.id}>
        <summary className="flex cursor-pointer list-none items-center gap-4 px-5 py-4 [&::-webkit-details-marker]:hidden">
          <span className="eyebrow w-32 shrink-0 truncate sm:w-56">{g.invitations?.code}</span>
